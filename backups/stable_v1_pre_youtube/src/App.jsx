...
export default function App() {
  ...
  // Permite forçar a atualização de todas as TVs instantaneamente via Painel Admin
  const [appStartTime] = useState(Date.now());
  useEffect(() => {
    if (settings?.system_reload_timestamp) {
      const reloadTime = parseInt(settings.system_reload_timestamp);
      if (reloadTime > appStartTime) {
        console.log("Comando de Reload Remoto recebido!", reloadTime);
        window.location.reload(true);
      }
    }
  }, [settings, appStartTime]);
  ...
  // MODO MANUTENÇÃO (Ativar conforme pedido do usuário)
